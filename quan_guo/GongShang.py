...
class GongShang(Searcher):
    ...
    def __init__(self):
        super(GongShang, self).__init__(True)
    ...
    def get_tag_a_from_page(self, keyword, flags=True):
        ...
        self.driver.get("http://www.gsxt.gov.cn/corp-query-homepage.html")
        name_in = self.driver.find_element_by_xpath(".//*[@id='keyword']")
        name_in.clear()
        name_in.send_keys(keyword)
        time.sleep(1) # 防止but1 的js未加载
        but1 = self.driver.find_element_by_xpath(".//*[@id='btn_query']")
        but1.click()
        time.sleep(1) # 等待图像加载
        self.info(u'正在查询%s' % keyword)
        self.geetest_verify(".//*[@class='search_result']")

        tree = self.driver.find_element_by_xpath('html').get_attribute('outerHTML')
        tree = html.fromstring(tree)
        self.driver.quit() # 退出浏览器
        result_name_list = tree.xpath(".//div[@class='search_list_item db']")
        print len(result_name_list)
        for i in range(len(result_name_list)):
            name = result_name_list[i].xpath("string(.//h1)").replace('(', u'（').replace(')', u'）').strip()
            print name
            if name != keyword:
        ...
        for i in range(len(result_name_list)):
            ...
            if flags:
                if name == keyword:
                    self.info(u'查询到指定公司')
                    self.tag_a = result_name_list[i].xpath('@href')[0].strip()
                    print self.tag_a
    ...
    def get_fen_zhi_ji_gou(self):
        ...
        for branch in brach_list:
            ...
            if branch.xpath('string(.//span[2]/span)'):
                branch_no = branch.xpath('string(.//span[2])/span').strip(u'（分）')
            else:
                branch_no = ''
...
if __name__ == '__main__':
    ...
    if args_dict:
        ...
        searcher.delete_tag_a_from_db(u'北京链家房地产经纪有限公司')
        searcher.submit_search_request(u"北京链家房地产经纪有限公司")

...
class GuangXiSearcher(Searcher):
    load_func_dict = {}
    cid = None

    def __init__(self):
        super(GuangXiSearcher, self).__init__(use_proxy=True)
        ...
        self.headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.3; WOW64; rv:45.0) Gecko/20100101 Firefox/45.0",
                        ...
                        "Referer": "http://gxqyxygs.gov.cn/searchList.jspx",
    ...
    def set_config(self):
        self.plugin_path = os.path.join(sys.path[0], '../guang_xi/ocr/pinyin/pinyin.bat')
    ...
    def download_yzm(self):
        image_url = 'http://gxqyxygs.gov.cn/validateCode.jspx'
    ...
    def get_tag_a_from_page(self, keyword):
        url = 'http://gxqyxygs.gov.cn/searchList.jspx'
        for t in range(20):
            yzm = self.get_yzm()
    ...
    def get_detail_info(self):
        url = 'http://gxqyxygs.gov.cn/businessPublicity.jspx'
    ...
    def get_gu_dong(self, table_element):
        ...
        urll = 'http://gxqyxygs.gov.cn/businessPublicity.jspx'
        ...
        for i in range(len(page_nums)):
            url = 'http://gxqyxygs.gov.cn/QueryInvList.jspx'
    ...
    def get_gu_dong_detail(self, id, table_element):
        detail_dict_list = []
        url = 'http://gxqyxygs.gov.cn/queryInvDetailAction.jspx'
    ...
    def get_bian_geng(self, table_element):
        ...
        if bsoup.select('div#altDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#altDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryAltList.jspx'
                params = {'mainId':self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_zhu_yao_ren_yuan(self, table_element):
        ...
        r = self.get_request(url=urll, params=para)
        bsoup = BeautifulSoup(r.text, 'lxml')
        pages=bsoup.select('div#memDiv + *')[0].text.strip()
        page_nums=re.findall(r'\d',pages)
        for i in range(len(page_nums)):
            url = 'http://gxqyxygs.gov.cn/QueryMemList.jspx'
            params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
            r = self.get_request(url = url, params = params)
            soup = BeautifulSoup(r.text, 'lxml')
            table_element = soup.find('table')
    ...
    def get_fen_zhi_ji_gou(self, table_element):
        ...
        if bsoup.select('div#childDiv > table')[0].text.strip() !=u'':
            ...
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryChildList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_dong_chan_di_ya(self, table_element):
        ...
        if bsoup.select('div#mortDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#mortDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryMortList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_gu_quan_chu_zhi(self, table_element):
        ...
        if bsoup.select('div#pledgeDiv > table')[0].text.strip() !=u'':
            ...
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryPledgeList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_xing_zheng_chu_fa(self, table_element):
        ...
        if bsoup.select('div#punDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#punDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryPunList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_jing_ying_yi_chang(self, table_element):
        ...
        if bsoup.select('div#excDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#excDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QueryExcList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_yan_zhong_wei_fa(self, table_element):
        ...
        if bsoup.select('div#serillDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#serillDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QuerySerillList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
    ...
    def get_chou_cha_jian_cha(self, table_element):
        ...
        if bsoup.select('div#spotCheckDiv > table')[0].text.strip() !=u'':
            pages=bsoup.select('div#spotCheckDiv + *')[0].text.strip()
            page_nums=re.findall(r'\d',pages)
            for i in range(len(page_nums)):
                url = 'http://gxqyxygs.gov.cn/QuerySpotCheckList.jspx'
                params = {'mainId': self.cid, 'pno': '%s' % (i+1)}
                r = self.get_request(url = url, params = params)
                soup = BeautifulSoup(r.text, 'lxml')
                table_element = soup.find('table')
...
if __name__ == '__main__':
    args_dict = get_args()
    args_dict = {'companyName': u'广西金桂浆纸业有限公司', 'accountId': '123', 'taskId': '456'}
